        ...
        for _domain, _sample_list in self.sample_list.items():
            for scan_id, itm in _sample_list.items():
                ...
                if self.normalize_op.mean is not None:
                    vol_info={'vol_vmin':img.min(),'vol_vmax':img.max(),'vol_mean':self.normalize_op.mean, 'vol_std':self.normalize_op.std}# NOTE there is a bug, 'vol_std':self.normalize_op.std
                else:
